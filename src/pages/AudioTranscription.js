...
const AudioTranscription = () => {
	...
	return (
		<div>
			<div style={topStyle}>
				<FaArrowAltCircleLeft style={{ color: "blue", cursor: "pointer" }} onClick={handleGoBack} />
			...
			{/* {showList ? (
					...
					{audioFiles.map((file) => (
						<div key={file.id} className="card">
							<div className="card-content">
								...
								<button className="btn right" onClick={() => handleStartTranscribing(file)}>
									Start
								</button>
							</div>
						</div>
				...
				<div style={centerStyle}>
					...
					<button style={{ display: "flex", alignItems: "center" }} onClick={handleDownloadClick}>
						<FaDownload style={{ marginRight: "8px" }} /> Start Transcribing
					</button>
					<button style={{ display: "flex", alignItems: "center", marginTop: "10px" }} onClick={handleGoBack}>
						Go Back
					</button>
				</div>
			)} */}
